...
group.add_option(
    "-C", "--configfile", dest="conf", default="", type=str, help="configuration file"
)
...
group.add_option(
    "-H", "--host", dest="host", default=None, type=str, help="listening address"
)

group.add_option(
    "-p", "--port", dest="port", default=None, type=int, help="listening port"
)
...
group.add_option(
    "-c",
    "--createconnection",
    dest="createconnection",
    nargs=8,
    metavar="username technology title host port database dbuser dbpassword",
...
if options.app:
    ...
    OmniDB.custom_settings.APP_TOKEN = "".join(
        random.choice(string.ascii_lowercase + string.digits) for i in range(50)
    )
    app_version = True
else:
    app_version = False
...
if options.homedir != "":
    if not os.path.exists(options.homedir):
        print(
            "Home directory does not exist. Please specify a directory that exists.",
            flush=True,
        )
        sys.exit()
    else:
...
if options.conf != "":
    ...
    if not os.path.exists(config_file):
        ...
        shutil.copyfile(
            os.path.join(OmniDB.custom_settings.BASE_DIR, "config.py"), config_file
        )
...
if hasattr(omnidb_settings, "SSL_CERTIFICATE_FILE"):
    ...
    if is_ssl and not os.path.exists(ssl_certificate_file):
        print(
            "Certificate file not found. Please specify a file that exists.", flush=True
        )
        sys.exit()
else:
...
old_db_file = dbfile = os.path.expanduser(
    os.path.join(OmniDB.custom_settings.HOME_DIR, "omnidb.db")
)
...
if options.listconnections:
    ...
    for conn in Connection.objects.filter(
        user=User.objects.get(username=options.listconnections)
    ):
        table.AddRow(
            [
                conn.id,
...
if platform.system() != "Windows":
    ...
    try:
        ...
        print(
            "OmniDB is already running pointing to directoy '{0}'.".format(
                OmniDB.custom_settings.HOME_DIR
            )
        )
        sys.exit()
        ...
        if num_attempts < 20:
            ...
            if parameters["is_ssl"]:
                ...
                ssl_ctx.load_cert_chain(
                    parameters["ssl_certificate_file"], parameters["ssl_key_file"]
                )
            ...
            print(
                "Starting server {0} at {1}:{2}{3}.".format(
                    OmniDB.settings.OMNIDB_VERSION,
                    parameters["listening_address"],
                    str(port),
            ...
            logger.info(
                "Starting server {0} at {1}:{2}.".format(
                    OmniDB.settings.OMNIDB_VERSION,
                    parameters["listening_address"],
                    str(port),
                )
            ...
            if not app_version:
                ...
                print(
                    "http://localhost:{0}/omnidb_login/?user=admin&pwd=admin&token={1}".format(
                        str(port), OmniDB.custom_settings.APP_TOKEN
                    ),
                    flush=True,
                )
